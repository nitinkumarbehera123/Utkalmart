...
if ( (isset($_POST['pid']) && $_POST['pid']!="")&& (isset($_POST['quantity']) && $_POST['quantity']!=""))
{
    ...
    $id = $_POST['pid'];
    $qty=$_POST['quantity'];
    $result = mysqli_query($con,"SELECT pId,pName,pPrice,pImage FROM `fresh_fruits` WHERE `pId`='$id'");
...
<?php
    ...
    ?>
    <div class="frontview">
        <img src="images/fresh-fruits.jpg" alt="">
    </div>

    <h2 class="sub-heading">All Fruits</h2>
    ...
    <section class="products" id="products">
        ...
        if((isset($_GET['searchInput'])))
        {
            ...
            $sql="SELECT * FROM `fresh_fruits` where pName LIKE '".$search."%'";
      
          
        }
        else{
            
            $sql="SELECT * FROM `fresh_fruits`";
        }
             $result = mysqli_query($con,$sql);
             while($row = mysqli_fetch_assoc($result)){
             echo "  <div class='box'>
